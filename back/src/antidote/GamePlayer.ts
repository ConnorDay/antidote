...
export class GamePlayer extends EmittablePlayer<events> {
	...
	constructor(name: string, socket: Socket, room: Antidote) {
		...
		this.socket.on("turnSelect", (select: TurnSelectObject) =>
			this.handleTurnSelect(select)
		);
	...
	handleTurnSelect(select: TurnSelectObject) {
		console.log(
			`Got a turn select from '${this.name}', action: ${select.action}, argument: ${select.argument}`
		);
		if (this.room.getCurrentPlayerTurnId() != this.id) {
			console.log(
				`Current turn does not belong to Player '${this.name}'`
			);
		...
		if (this.room.current_action !== undefined) {
			console.log(
				`Player '${this.name}' sent a valid select, but an select is already set.`
			);
			this.socket.emit("error", {
				message:
					"Valid select, but game already has an select designated",
			});
			return;
		}
		...
		switch (select.action) {
			...
			default:
				...
				this.socket.emit("error", {
					message: "Unhandled action passed.",
				});
				break;
		}
	}
	...
	handlePassAction(direction?: string) {
		...
		if (direction !== "left" && direction !== "right") {
			console.warn(
				`Player '${this.name}' did not provide a valid direction: ${direction}`
			);
			this.socket.emit("error", {
	...
	handleTradeAction(target?: string) {
		...
		const target_player = this.room.connected_players.find(
			(p) => p.id === target
		);
		if (target_player === undefined) {
			console.warn(`Could not find a player id with id '${target}'`);
			this.socket.emit("error", {
				message: `Could not find a player id with id '${target}'`,
			});
			return;
		}

		console.log(
			`Player '${this.name}' initiated a trade request with Player '${target_player.name}'`
		);
		this.waiting = false;
	...
	handleUseAction(card?: string, target_type?: string, target_id?: string) {
		...
		if (found_card === undefined) {
			console.error(
				`Could not find a card with id '${card}' in the hand of Player '${this.name}'`
			);
			this.socket.emit("error", {
				message: "Could not find provided card",
			});
			return;
		}
		...
		if (target_type === undefined || card === null) {
			console.error(
				`Player '${this.name}' did not provide a target type`
			);
		...
		if (target_type !== "card" && target_type !== "player") {
			console.error(
				`Player '${this.name}' provided an invalid target type '${target_type}'`
			);
	...
	async queryHand(
		...
		return new Promise<Card | undefined>((accept, reject) => {
			const responseCallback = (card_id: string | null) => {
				console.log(
					`Got a query hand response from Player '${this.name}': '${card_id}'`
				);
				if (card_id === null && !can_reject) {
					console.warn(
						`Player '${this.name}' attempted to reject an unrejectable query.`
					);
				...
				if (card_id !== null) {
					...
					if (target_card === undefined) {
						console.warn(
							`Could not find card with id '${card_id}' for Player '${this.name}'`
						);
						this.socket.emit("error", {
							message: `Could not find card with id '${card_id}'`,
						});
						return;
					}
				}
